...
original_pdf = os.path.join (current_folder, f"DIPLOMA_Especialista.pdf")

def generatePDF(alumno, dni, fecha_inicio, fecha_fin, docente1, docente2, docente3):
    packet = io.BytesIO()
    ...
    #Página 1
    c.setFont('arial', 12)
    c.drawString(106, 355, alumno)
    c.drawString(178, 339, dni)

    c.setFont('arial', 11)
    c.drawString(106, 230, f"Realizado del {fecha_inicio} al {fecha_fin} en modalidad presencial y online")

    c.setFont('arial', 11)
    c.drawString(412, 205.5, docente1)
    c.drawString(445, 192.5, docente2)
    c.drawString(333, 180, docente3)

    c.setFont('arial', 11)
    c.drawString(268, 155, fecha_fin)
...
for i in range (1, hoja.nrows):
    ...
    docente3 = hoja.cell_value(i, 6)
    
    generatePDF(alumno, dni, fecha_inicio, fecha_fin, docente1, docente2, docente3)
